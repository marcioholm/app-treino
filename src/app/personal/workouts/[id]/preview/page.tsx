...
const groupColors: Record<string, string> = {
    ...
    'Costas': 'bg-[#D4537E]/20 text-blue-700',
        ...
        <div className="min-h-screen bg-black pb-20">
            ...
            <div className="bg-gradient-to-br from-blue-600 to-blue-800 text-white p-6">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="bg-[#111111]/20 px-3 py-1 rounded-full text-xs font-medium">
                        Rascunho
                    </span>
                </div>
            ...
            <div className="px-4 -mt-4">
                <div className="bg-[#111111] rounded-xl shadow-sm border border-[#333333] p-4 flex justify-around">
                    <div className="text-center">
                        <p className="text-2xl font-bold text-white">{workout.sessions[0].exercises.length}</p>
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-xs text-gray-400">Séries Totais</p>
                    </div>
                    <div className="text-center">
            ...
            <div className="p-4 space-y-3">
                ...
                {workout.sessions[0].exercises.map((exercise, index) => (
                    <div key={index} className="bg-[#111111] rounded-xl border border-[#333333] shadow-sm overflow-hidden">
                        <div 
                            className="p-4 flex items-center gap-4 cursor-pointer"
                            onClick={() => setExpandedExercise(expandedExercise === exercise.name ? null : exercise.name)}
                        >
                            <div className="w-8 h-8 bg-[#1a1a1a] rounded-lg flex items-center justify-center text-gray-400 font-bold text-sm">
                                {index + 1}
                            </div>
                            <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                    <h3 className="font-semibold text-white">{exercise.name}</h3>
                                    <span className={`text-xs px-2 py-0.5 rounded-full ${groupColors[exercise.group] || 'bg-[#1a1a1a] text-gray-300'}`}>
                                        {exercise.group}
                                    </span>
                                </div>
                            ...
                            <div className="text-right">
                                <p className="font-bold text-white">{exercise.sets}x{exercise.reps}</p>
                                <p className="text-xs text-gray-400">{exercise.restTime}s pausa</p>
                            </div>
                        ...
                        {expandedExercise === exercise.name && (
                            <div className="px-4 pb-4 pt-0 border-t border-[#333333]">
                                <div className="mt-3 grid grid-cols-3 gap-3">
                                    ...
                                    <div className="bg-black rounded-lg p-3 text-center">
                                        ...
                                        <p className="font-bold text-white">{exercise.restTime}s</p>
                                    </div>
                                </div>
                                {exercise.notes && (
            ...
            {/* Action Buttons */}
            <div className="p-4 fixed bottom-0 left-0 right-0 bg-[#111111] border-t border-[#333333]">
                <div className="flex gap-3">
                    <button className="flex-1 py-3 px-4 bg-[#1a1a1a] text-gray-300 font-medium rounded-xl hover:bg-[#333333] transition-colors">
                        Editar
                    </button>
                    <button className="flex-1 py-3 px-4 bg-[#D4537E] text-white font-medium rounded-xl hover:bg-[#993556] transition-colors">
                        Publicar
                    </button>
                </div>
            </div>
